...
template <class T> inline void
HICEUtil::Monitor<T>::unlock() const
{
    if(_mutex.willUnlock())
    {
	//
...
HICEUtil::Monitor<T>::tryLock() const
{
...
template <class T> inline bool
HICEUtil::Monitor<T>::timedWait(const Time& timeout) const
{
    //
    // Perform any pending notifies.
    //
...
template <class T> inline void
HICEUtil::Monitor<T>::notifyImpl(int nnotify) const
{
    //
