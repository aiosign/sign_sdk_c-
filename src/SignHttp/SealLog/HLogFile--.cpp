...
void log_init_real(const char* filename,bool bdirectlog,unsigned iminflushnum, int iOutputLevel, int iDisplayWhere)
{
	...
	strfilename = HCommonFun::GetMyExeFullPath();
...
WLog log_get_block(const char* filename,int linenum, int level)
{
	...
	{
		perror("You should call log_init() first\n");
		log_init_real(NULL,true,10,LOG_INFO,DISPLAY_FILE);
		if(g_Log == NULL)
		{
